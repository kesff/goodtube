	...
	function goodTube_player_positionAndSize() {
		...
		if (goodTube_fallback) {
			if (!goodTube_clearedPlayer) {
				console.log('fallback is active, clearing player');
		...
		// Otherwise, the "hide and mute ads" fallback is inactive
			...
			if (!youtubePlayer || youtubePlayer.offsetHeight <= 0) {
				...
				positionElement = '.html5-video-player';
			}
			...
			if (youtubePlayer && youtubePlayer.offsetHeight > 0) {
				...
				console.log('position and size working, showing player. position element is ', positionElement);
			}
			else {
				console.log('cant find player element for positioning');
			}
		}
	}
	...
	*/
	...
	function goodTube_receiveMessage(event) {
		...
		// Player iframe has loaded
			...
			console.log('player loaded, showing with css');
		}
